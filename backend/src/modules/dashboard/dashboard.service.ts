  ...
  async getKpis() {
    const [
      ...
      this.databaseService.query<any>(
        `SELECT COUNT(*) as c FROM public.properties WHERE status IN ('En revisión','Activa','disponible')`,
        {},
      ),
      this.databaseService.query<any>(
        `SELECT COUNT(*) as c FROM public.properties WHERE (contrato_comision_firmado IS NULL OR contrato_comision_firmado = 'false') AND status NOT IN ('vendida','rentada')`,
        {},
      ),
      this.databaseService.query<any>(
        `SELECT COUNT(*) as c FROM public.properties`,
        {},
      ),
      this.databaseService.query<any>(
        `SELECT COUNT(*) as c FROM public.operations WHERE status IN ('Solicitado','En revisión')`,
        {},
      ),
      this.databaseService.query<any>(
        `SELECT COUNT(*) as c FROM public.operations WHERE status = 'Validado por administración'`,
        {},
      ),
      this.databaseService.query<any>(
        `SELECT COUNT(*) as c FROM public.operations`,
        {},
      ),
      this.databaseService.query<any>(
        `SELECT COALESCE(SUM(amount),0) as t, COUNT(*) as c FROM public.commissions WHERE estatus_comision IN ('Calculada','Pendiente validación') OR (estatus_comision IS NULL AND status != 'Pagada')`,
        {},
      ),
      this.databaseService.query<any>(
        `SELECT COALESCE(SUM(monto_neto_asesor),0) as t FROM public.commissions WHERE estatus_comision IN ('Liberada','Pagada')`,
        {},
      ),
      this.databaseService.query<any>(
        `SELECT COALESCE(SUM(monto_inmobiliaria),0) as t FROM public.commissions WHERE estatus_comision IN ('Liberada','Pagada')`,
        {},
      ),
      this.databaseService.query<any>(
        `SELECT COUNT(*) as c FROM public.advisors WHERE status = 'Activo'`,
        {},
      ),
      this.databaseService.query<any>(
        `SELECT COUNT(*) as c FROM public.advisors WHERE pasa_por_mentoria = 'true' AND status = 'Activo'`,
        {},
      ),
      this.databaseService.query<any>(
        `SELECT COUNT(*) as c, COALESCE(SUM(monto_neto_asesor),0) as t FROM public.commissions WHERE estatus_comision = 'Bloqueada'`,
        {},
      ),
    ]);
