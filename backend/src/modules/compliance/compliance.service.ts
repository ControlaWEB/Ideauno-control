...
export class ComplianceService {
  ...
  async findOne(id: string) {
    ...
    if (!rows.length)
      throw new NotFoundException(
        `Expediente de cumplimiento ${id} no encontrado.`,
      );
    return rows[0];
  }
  ...
  async updateCase(
    ...
    if (dto.rfcValid !== undefined) {
      setClauses.push('rfc_valid = @rfcValid');
      params.rfcValid = dto.rfcValid;
    }
    if (dto.identificationValid !== undefined) {
      setClauses.push('identification_valid = @identificationValid');
      params.identificationValid = dto.identificationValid;
    }
    if (dto.pepCheck !== undefined) {
      setClauses.push('pep_check = @pepCheck');
      params.pepCheck = dto.pepCheck;
    }
    if (dto.observations !== undefined) {
      setClauses.push('observations = @observations');
      params.observations = dto.observations;
    }
    if (dto.alertTrigger !== undefined) {
      setClauses.push('alert_trigger = @alertTrigger');
      params.alertTrigger = dto.alertTrigger;
    }
    ...
    if (dto.status === 'validado' && existing.status !== 'validado') {
      ...
      await this.databaseService.query(unlockSql, {
        opId: existing.operation_id,
      });

      const updateOp = `UPDATE public.operations SET compliance_status = 'Verde' WHERE id = @opId`;
      await this.databaseService.query(updateOp, {
        opId: existing.operation_id,
      });
    }
  ...
  async getKpis() {
    const [overThreshold, pendingSat, criticalAlerts, thresholdInfo] =
      await Promise.all([
        this.databaseService.query<any>(
          `SELECT COUNT(*) as count FROM public.compliance_cases`,
          {},
        ),
        this.databaseService.query<any>(
          `SELECT COUNT(*) as count FROM public.compliance_cases WHERE status = 'pendiente_docs' OR status = 'en_integracion'`,
          {},
        ),
        this.databaseService.query<any>(
          `SELECT COUNT(*) as count FROM public.compliance_cases WHERE status = 'bloqueado'`,
          {},
        ),
        Promise.resolve([
          {
